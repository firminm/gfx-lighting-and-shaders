...
class Sketch(CanvasBase):
    ...
    # Toggling specular/diffuse/ambient
    specularOn = True
    diffuseOn  = True
    ambientOn  = True
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if chr(keycode) in "sS":
            ...
            self.shaderProg.setSpecular(self.specularOn)
            print('Specular light set to', self.specularOn)

        if chr(keycode) in "dD":
            self.diffuseOn = not self.diffuseOn
            self.shaderProg.setDiffuse(self.diffuseOn)
            print('Diffuse light set to', self.diffuseOn)

        if chr(keycode) in "aA":
            self.ambientOn = not self.ambientOn
            self.shaderProg.setAmbient(self.ambientOn)
            print('Ambient light set to', self.ambientOn)
        ...
        if chr(keycode) in "bB":
            self.switchScene(SceneFive(self.shaderProg))
